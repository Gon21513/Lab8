...
void main(void){
    setup();
    char txt[] = "QUE DICE REY";//cadena de caracteres
    //envio de datos
    while (1){
        
        __delay_ms(500);
        cadena(txt);// Llamar a la función cadena() con la cadena de caracteres a enviar
...
void cadena(char txt[]) {
    for (uint8_t i = 0; txt[i] != '\0'; i++) { // Recorrer la cadena de caracteres hasta encontrar  '\0'
        while (!PIR1bits.TXIF); // si txif esta disponible 
        TXREG = txt[i]; // Enviar el caracter por comunicación serial
    }
}